...
choice = st.sidebar.selectbox("", ('Accueil','Le cinéma en quelques chiffres','Les acteurs dans le cinéma','Le top 10',
...
if choice == 'Accueil':
    ...
    components.html("<p style='color:white;font-family:verdana; font-size:20px; text-align: left'><i>Bienvenue sur notre tout nouveau site internet, vous pouvez dès à présent parcourir nos différents onglets pour trouver LE FILM qu'il vous faut. Notre tout nouvel outil vous guidera vers une sélection personnalisée.</i></p>")
    ...
    fig1 = px.bar(new_compteur, x=new_compteur['Genres'], y=new_compteur['Nb films'], labels=dict(
    ...
    fig1.update_layout(title='<b>Nombre de films par genre</b>',
    ...
    fig2 = px.pie(b, values=0, names='index', labels='index',
                  color_discrete_sequence=px.colors.sequential.Teal)
    fig2.update_layout(width=800, height=500)
    fig2.update_traces(textposition='inside', textinfo='percent+label')
    fig2.update_layout(title='<b>Répartition des films par genre</b>',
                       title_x=0.5, title_font_family="Verdana")
    ...
    fig3 = px.bar(e, x='index', y=0, labels=dict(
        x="Pays", y="Nb Films"), color=0, height=400, color_continuous_scale=px.colors.sequential.Teal)
    fig3.layout.coloraxis.showscale = False
    fig3.update_layout(title='<b>Nombre de films par pays</b>',
    ...
    fig4 = px.pie(c, values=0, names='index', labels='index',
                  color_discrete_sequence=px.colors.sequential.Teal)
    fig4.update_traces(textposition='inside', textinfo='percent+label')
    fig4.update_layout(width=800, height=500)
    fig4.update_layout(title="<b>Répartition des films par pays d'origine</b>",
                       title_x=0.5, title_font_family="Verdana")
    ...
    choix_pays = st.selectbox('Choisir le pays', liste_pays)
    ...
    if choix_pays != 'Tous':
        ...
        fig41 = px.box(df_bases_filmsFR_country, x="startYear", y="averageRating")
        fig41.update_layout(title_text="<b>Evolution de la répartition des notes", title_x=0.5, title_font_family="Verdana")
        fig41.update_layout({'plot_bgcolor': 'rgba(0,0,0,0)','paper_bgcolor': 'rgba(0,0,0,0)'})
        fig41.update_xaxes(showgrid=False, gridwidth=1, gridcolor='black', linecolor ='rgba(0,0,0,0)')
        fig41.update_yaxes(showgrid=False, gridwidth=1, gridcolor='black')
        fig41.update_yaxes(title_text="<b>Notes", dtick = 2, range = [0,10])
        fig41.update_xaxes(title_text="<b>Années")
        ...
        fig41.update_layout(title_text="<b>Evolution de la répartition des notes", title_x=0.5, title_font_family="Verdana")
        fig41.update_layout({'plot_bgcolor': 'rgba(0,0,0,0)','paper_bgcolor': 'rgba(0,0,0,0)'})
        fig41.update_xaxes(showgrid=False, gridwidth=1, gridcolor='black')
        fig41.update_yaxes(showgrid=False, gridwidth=1, gridcolor='black')
        fig41.update_yaxes(title_text="<b>Notes", dtick = 2, range = [0,10])
        fig41.update_xaxes(title_text="<b>Années", linecolor = 'rgba(0,0,0,0)')
        st.write(fig41)
    ...
    df_bases_acteur_visu2 = filtered_data2.drop_duplicates(subset=['primaryName'])
    ...
    c_final['visu'] = c_final['lien_image'].apply(lambda x: link_preview(x).image)
    ...
    fig7 = px.bar(all_data_title, x='title', y='acteurs', color='title', text="title", height=400, color_continuous_scale=px.colors.sequential.Teal, orientation='h')
    fig7.layout.coloraxis.showscale = False
    fig7.update_layout(yaxis={'categoryorder':'total ascending'}, title='<b>La suite du classement</b>',
                       title_x=0.5, title_font_family="Verdana")                      
    fig7.update_traces(texttemplate='%{text:.2s}', textposition='outside')
    fig7.update_layout(uniformtext_minsize=14, uniformtext_mode='hide')
    fig7.update_layout( yaxis = dict( tickfont = dict(size=16)))
    fig7.update_layout({'plot_bgcolor': 'rgba(0,0,0,0)',
                       'paper_bgcolor': 'rgba(0,0,0,0)', })
    fig7.update_xaxes(title_text="Nb Films")
    fig7.update_yaxes(title_text="")
    fig7.update_xaxes(showgrid=False, gridwidth=1, gridcolor='black')
    fig7.update_yaxes(showgrid=False, gridwidth=1, gridcolor='black')
    st.write(fig7) 

    fig8 = px.bar(all_data_note, x='averageRating', y='acteurs', color='averageRating', text="averageRating", height=400, color_continuous_scale=px.colors.sequential.Teal, orientation='h')
    fig8.layout.coloraxis.showscale = False
    fig8.update_layout(yaxis={'categoryorder':'total ascending'}, title='<b>Acteurs avec les mieux notés</b>',
                       title_x=0.5, title_font_family="Verdana")
    fig8.update_traces(texttemplate='%{text:.2s}', textposition='outside')
    fig8.update_layout(uniformtext_minsize=14, uniformtext_mode='hide')
    fig8.update_layout({'plot_bgcolor': 'rgba(0,0,0,0)',
                       'paper_bgcolor': 'rgba(0,0,0,0)', })
    fig8.update_xaxes(range=[5,8.5])
    fig8.update_xaxes(tick0=0, dtick=0.5)
    fig8.update_layout(yaxis = dict( tickfont = dict(size=16)))
    fig8.update_xaxes(title_text="Notes moyennes")
    fig8.update_yaxes(title_text="")
    fig8.update_xaxes(showgrid=False, gridwidth=1, gridcolor='black')
    fig8.update_yaxes(showgrid=False, gridwidth=1, gridcolor='black')
    st.write(fig8)
    ...
    st.write("")
    st.markdown("<p class = 'p4'>Afin de célébrer la réouverture de notre cinéma, nous vous proposons de revivre chaque mois le meilleur du cinéma de ces 5 dernières décénnies!</p>", unsafe_allow_html=True)
